...
Move Test::get_move(State *state, int depth){
  ...
  TestNode* root = build_tree(state->legal_actions.at(0), state, 0, depth, NULL);
...
TestNode* build_tree(Move move, State* cur_state, int cur_depth, int total_depth, TestNode* parent_node) {
  ...
  TestNode* new_node = build_node(move, cur_state, cur_state->legal_actions.size(), cur_depth);
