 ...
 * @return Move 
 */
Move sub_AlphaBeta::get_move(State *state, int depth){
  if(!state->legal_actions.size())
    state->get_legal_actions();
  sub_AlphaBetaNode* root = build_sub_alpha_beta_tree(state->legal_actions.at(0), state, 0, depth, NULL);
  int goal_score = get_sub_alpha_beta_best_score(root);
  Move goal_move;
...
sub_AlphaBetaNode* build_sub_alpha_beta_tree(Move move, State* cur_state, int cur_depth, int total_depth, sub_AlphaBetaNode* parent_node) {
  ...
  sub_AlphaBetaNode* new_node = build_sub_alpha_beta_node(move, cur_state, cur_state->legal_actions.size(), cur_depth);
  ...
  for(int i = 0; i < new_node->child_num; i++) {
    ...
    new_node->child[i] = build_sub_alpha_beta_tree(new_node->state->legal_actions.at(i), new_state, cur_depth + 1, total_depth, new_node);
...
int get_sub_alpha_beta_best_score(sub_AlphaBetaNode* root) {
    ...
    if((root->depth % 2) == 0) {  // currently in player's state
        for(int i = 0; i < root->child_num; i++) {
            store_score = get_sub_alpha_beta_best_score(root->child[i]);
...
void delete_sub_alpha_beta_tree(sub_AlphaBetaNode* root) {
    if(root->child_num) {
        for(int i = 0; i < root->child_num; i++) delete_sub_alpha_beta_tree(root->child[i]);
    }
